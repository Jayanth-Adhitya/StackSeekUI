...
export const analyzeError: RequestHandler = async (req, res) => {
  try {
    const { repositoryId, errorMessage, codeSnippet } =
    ...
    res.status(501).json({
      success: false,
      error:
        "Error analysis service is not yet implemented. Please connect your repository and configure the AI analysis backend.",
    });
  } catch (error) {
